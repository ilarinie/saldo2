...
export class RootState {
    ...
    private setupWebsocket = ()  => {
        const newWebsocket = new WebSocket(process.env.REACT_APP_WEBSOCKET_URI || 'ws://localhost:3001/ws');
    ...
    private setPurchasesAndCalculateTotal = (purchases: Purchase[]) => {
        runInAction(() => {
            ...
            let cumTotal = 0;
            this.purchasesWithCumulativeTotal = [ ...purchases].reverse().map((p, index)=> {
                cumTotal += p.amount;
                return {
                    cumTotal,
                    ...p
    ...
    createPurchase = async (amount: number, description: string) => {
        if (amount && description) {
            try {
                const res = await axios.post<Purchase>("/api/purchases", { amount, description });
