...
export const budgetApi = createApi({
    ...
    endpoints: (builder) => ({
        getBudgets: builder.query<{ ids: string[], map: { [key: string]: Budget } }, void>({
            query: () => '/budgets',
            transformResponse: (response: { message: string, payload: Budget[] }) => {
                const ids: string[] = []
...
const updateBudgetInCacheAction = (budget: Budget) => budgetApi.util.updateQueryData('getBudgets', undefined, (state) => {
