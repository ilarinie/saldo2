...
export const AddPurchase = observer(({ open, onClose, currentUser, budget }: AddPurchaseProps) => {
  ...
  const [benefactors, setBenefactors] = useState(
    initBenefactors(amount || 0, budget.type === 'saldo' ? 'saldo' : 'even-split', selectedPayerId, budget?.userMap, budget?.allIds)
  )
  ...
  const [createPurchase, { isLoading, error, data }] = useCreatePurchaseMutation()
    ...
    <SaldoModal open={open} onClose={onClose}>
      ...
      <CardContent>
        <StyledTextField
          ...
          label='Amount'
          value={amount}
          onChange={event => setAmount(currency(event.target.value).value || '')}
          InputProps={{
            startAdornment: <InputAdornment position='start'>€</InputAdornment>,
          }}
        />
        <StyledTextField type='text' label='Description' value={description} onChange={event => setDescription(event.target.value)} />
        ...
        <BenefactorEditor
          onDefaultModeChanged={mode => {
            setBenefactors(initBenefactors(amount || 0, mode, selectedPayerId, budget.userMap, budget.allIds))
          }}
