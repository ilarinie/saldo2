...
const generateBenefactors = (budget: Budget, payerId: string, receiverId: string, amount: number): Benefactor[] =>
  budget.allIds.map(b => ({
...
export const AddTransfer = ({ budget, open, onClose, currentUser }: AddTransferProps) => {
  const [{ selectedPayer, selectedReceiver, amount, selectableReceivers }, setNewTransferState] = useState<NewTransferState>({
    selectedPayer: currentUser._id,
    selectedReceiver: budget?.allIds[0] === currentUser._id ? budget.allIds[1] || budget.allIds[0] : budget.allIds[0],
    amount: undefined,
    selectableReceivers: [],
  })
  ...
  const onPayerChange = (newPayerId: string) => {
    const newSelectableReceivers = budget.allIds.filter(s => s !== newPayerId)
