...
const getPayer = (purchase: Purchase) => purchase.benefactors.filter(b => b.amountPaid > 0)[0].user
...
export const BudgetExpanded = ({ budget, deletePurchase }: BudgetExpandedProps) => {
  const [selectedPurchase, setSelectedPurcase] = useState(undefined as undefined | Purchase)
    ...
    <CardContent>
      ...
      <List>
        {budget.purchases.slice(0, 5).map(p => (
          <PurchaseItem purchase={p} key={p._id} deletePurchase={deletePurchase} onPurchaseSelected={setSelectedPurcase} />
        ))}
      </List>
          ...
          {selectedPurchase && (
            <>
              <CardHeader
                ...
                action={
                  <IconButton aria-label='close' onClick={() => setSelectedPurcase(undefined)}>
                    <CloseRoundedIcon />
                  </IconButton>
                }
              />
              <CardContent>
                ...
                <Typography variant='bigCurrency'>{currency(selectedPurchase.amount).format(CurrencyFormatOptions)}</Typography>
                  ...
                  <TableBody>
                    {selectedPurchase.benefactors.map(benefactor => (
                      ...
                      >
                        ...
                        <TableCell align='right'>{currency(benefactor.amountBenefitted).format(CurrencyFormatOptions)}</TableCell>
  ...
  return (
    ...
    >
      ...
      <ListItemText
        ...
        secondary={`${currency(purchase.amount).format(CurrencyFormatOptions)} - ${new Date(purchase.createdAt).toLocaleTimeString()}`}
      />
      <Menu
        id='basic-menu'
        anchorEl={anchorEl}
