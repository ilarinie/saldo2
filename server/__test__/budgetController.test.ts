...
describe('Endpoints spec', () => {
  beforeEach(async () => {
    try {
      await connectToDatabase(process.env.TEST_MONGO_URI + 'budgetController')
  ...
  describe('Gets specific budget', () => {
    it('finds budget if user belongs to it', async () => {
      const res = await request(app).get(`/api/budgets/${TEST_SALDO_BUDGET_ID}`)
    ...
    it('returns 400 if budget not accesible', async () => {
      const res = await request(app).get(`/api/budgets/${TEST_BUDGET_2_ID}`)
      expect(res.status).toBe(400)
      expect(res.body.message).toBe('Budget not accesible for user 61391c620c7d2d6d8db0d354')
    })
  })
  ...
  describe('Updates a budget', () => {
    it('updates budget name', async () => {
      const res = await request(app).put(`/api/budgets/${TEST_SALDO_BUDGET_ID}`).send({
        name: 'newBudget',
      })
      expect(res.status).toBe(200)
      const budget = res.body.resp
  ...
  describe('Adds new members', () => {
    it('adds manual members with proper request', async () => {
      const res = await request(app).post(`/api/budgets/${TEST_SALDO_BUDGET_ID}/addnewusers`).send({
    ...
    it('returns 406 with incorrect params', async () => {
      ...
      const error = res.body.payload[0]
    ...
    it('returns 406 with incorrect budget id', async () => {
      const res = await request(app).post(`/api/budgets/höpönpöppöö/addnewusers`).send({
        username: 'james bond',
      })
      expect(res.status).toBe(406)
    })

    it('returns 400 with incorrect budget id', async () => {
      const res = await request(app).post(`/api/budgets/615f417a3333fbdcc8f98f2c/addnewusers`).send({
        username: 'james bond',
      })
      expect(res.status).toBe(400)
    })
