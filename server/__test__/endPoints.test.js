...
describe('Endpoints spec', () => {
    ...
    describe('GET /api/purchases', () => {
        it('Should respond with a valid request', async () => {
            const res = await request(app).get('/api/purchases').set('Authorization', testSecret);
    ...
    describe('POST /api/purchases', () => {
        it('Should create a purchase with a valid request', async () => {
            const res = await request(app).post('/api/purchases').send({ amount: 2, description: 'testing'}).set('Authorization', testSecret);
            expect(res.status).toEqual(200);
        ...
        it('Should not create a purchase with a invalid request', async () => {
            const res = await request(app).post('/api/purchases').send({ amount: 2 }).set('Authorization', testSecret);
        ...
        it('Should not create a purchase with a valid request, unauthorized', async () => {
            const res = await request(app).post('/api/purchases').send({ amount: 2 }).set('Authorization', 'wrongSecret');
            expect(res.status).toEqual(401);
        });
    })
    ...
    describe('DELETE /api/purchases/:id', () => {
        it('Should delete a purchase with a valid request', async () => {
            ...
            const res = await request(app).delete('/api/purchases/' + _id).set('Authorization', testSecret);
        ...
        it('Should not delete a purchase with a unauthorized request', async () => {
            const { _id } = await PurchaseModel.findOne({});
            const res = await request(app).delete('/api/purchases/' + _id).set('Authorization', 'wrongSecret');
            expect(res.status).toEqual(401);
            const { deleted } = await PurchaseModel.findOne({ _id });
